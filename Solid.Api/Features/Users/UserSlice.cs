...
public static class UserSlice
{
    public static IEndpointRouteBuilder MapUserSlice(this IEndpointRouteBuilder api)
    {
        ...
        api.MapGet("/users/{userId:long}", Show);
        api.MapPut("/profile", UpdateProfile);
        api.MapPut("/profile/fcm-token", UpdateFcmToken)
            .Accepts<FcmTokenRequest>("application/json", "application/x-www-form-urlencoded", "multipart/form-data");
        api.MapGet("/instructors", Instructors);
        api.MapGet("/instructors/{userId:long}", ShowInstructor);
        api.MapPost("/instructors", CreateInstructor)
            .Accepts<CreateInstructorRequest>("application/json", "application/x-www-form-urlencoded", "multipart/form-data");
    ...
    private static async Task<IResult> UpdateFcmToken(IAuthContext auth, HttpRequest httpRequest, IUserRepository userRepository)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.fcm_token))
            return ApiResponse.Fail("fcm_token is required.", StatusCodes.Status422UnprocessableEntity);

        await userRepository.UpdateFcmTokenAsync(auth.UserId, request.fcm_token);

        return ApiResponse.Ok(message: "FCM token updated successfully.");
    }
...
public sealed record FcmTokenRequest(string fcm_token);
