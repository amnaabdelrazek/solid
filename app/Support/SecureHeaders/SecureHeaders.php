...
namespace App\Support\SecureHeaders;

use App\Support\SecureHeaders\Builders\ClearSiteDataBuilder;
use App\Support\SecureHeaders\Builders\ContentSecurityPolicyBuilder;
use App\Support\SecureHeaders\Builders\ExpectCertificateTransparencyBuilder;
use App\Support\SecureHeaders\Builders\PermissionsPolicyBuilder;
use App\Support\SecureHeaders\Builders\StrictTransportSecurityBuilder;
use Exception;
...
class SecureHeaders
{
    ...
    public static function removeNonce(?string $target = null, ?string $nonce = null)
    {
        if ($target === null) {
            ...
            if ($nonce === null) {
                self::$nonces[$target] = [];
            } elseif (($idx = array_search($nonce, self::$nonces[$target])) !== false) {
    ...
    protected function hsts(): array
    {
        ...
        if (! ($config['enable'] ?? false) && ! in_array(config('app.env'), $config['environments'] ?? [])) {
            return [];
        }
